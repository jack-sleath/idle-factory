...
prod build), `npm run simulate` (balancing report), `npm run ladder` (the pricing
tier ladder; `-- write` applies it to the data). Tests live in `test/`.
...
villager chain eats far more of this than a short sell line),
`buffInvestmentFraction` (share of money/build-budget routed into the villager
buff pipeline vs. sell lines), and `duplicateLines` (false = a collector who only
ever builds one line per item). `PRESET_PROFILES` crosses four cadences with
seller/buffs, plus two single-strategy profiles:

- **`Villager-max`** — 85% of income into the town-hall pipeline, banking *every*
  specialist rather than only the ones an auto-seller economy rewards. Not 100%:
  a pure villager player builds no sell lines, so income never leaves the starter
  ore and the pipeline stalls.
- **`Completionist`** — one line per item, never a second copy. Its curve is
  content coverage, not net worth, which is what `itemMilestones`,
  `distinctItemsBuilt` and `allItemsDay` on `SimResult` measure: when each item
  came online, and the day the last one did. The report prints the collector's
  per-day unlock pace, which is the quickest read on "how much content is there,
  and how fast does it run out?"

**Saving for the next unlock.** A purely greedy shopper spends its whole balance
every session and so never accumulates a lump sum — which made a player who
shops five times a day reach a big-ticket spawner *later* than one who shops daily
and banks 24h of income first. `REINVEST_SHARE` (0.5) caps how much of the balance
goes on duplicates while some line is still out of reach; the rest is held for it.
Without this the end-game metric is not monotonic in play frequency and a tiered
ladder cannot be measured at all.
...
The economy is priced by a **tier ladder**, generated by `scripts/ladder.ts`
(`npm run ladder`) and modelled on Cookie Clicker's building ladder. Read that
file before changing any price: it is the source of truth, and hand-editing a
cost or value in the JSON puts you outside the ladder.

### The ladder

Every spawner belongs to a tier. Each tier's raw items carry a hand-picked value
(flavour — a lemon is not a diamond), and each spawner's cost is then **derived**
as `payback_target(tier) × that line's income`:

| tier | spawner cost | raw items | line payback |
| ---- | ------------ | --------- | ------------ |
| T1 | $2–3k | ore, water, log ($1) | 12k ticks |
| T2 | $8–12k | crops ($2–3) | 20k ticks |
| T3 | $31–41k | fruit, egg, milk, wool ($5–8) | 36k ticks |
| T4 | $500k | pumpkin, honey ($15–20) | 200k ticks |
| T5 | $2.1–2.2M | silver, gold ore ($30–40) | 550k ticks |
| T6 | $11M | sapphire, emerald ($110–130) | 1.4M ticks |
| T7 | $43M | ruby, diamond ($320–450) | 2.7M ticks |

Two properties follow, and they are the whole point:

1. **Within a tier, every spawner earns the same return per coin**, so there is no
   single best farm to spam. (Before the ladder, a $48k beehive out-earned every
   other farm line by 3× — a pure tuning accident.)
2. **Across tiers, payback gets steadily worse.** A higher tier is a *goal*, not
   an efficiency play — you buy the ruby deposit because it is the next rung, the
   way a Cookie Clicker player buys a Prism. This is what makes the ladder read as
   progression rather than as a menu.

`costGrowth` stays at **1.15** on every spawner — the same figure Cookie Clicker
uses per purchase, and the reason no single line can be duplicated to infinity.
Plumbing (belts, sellers, storage, processors, combiners) stays flat.

The **village hut ($200k)** and **town hall ($1M)** are priced by hand, as goals
rather than producers: they consume items and emit villagers, so they never pay
back directly. See #4 below for why their price is a weak lever.

### Hard-won facts about the curve

1. **Income compounds; a one-time price does not gate it.** Sweeping the hut/hall
   price over four orders of magnitude barely moved the end-game day — at 5,000×
   their old price the town was still worth ×28 sell. A fixed cost only delays
   *when* a compounding engine starts, never how strong it gets. Costs gate
   progress when they are on the order of *daily income*, which is why the top of
   the ladder is in the tens of millions.

2. **Cheap spawners cannot be slow to pay back.** With $1 as the floor item value,
   a cheap T1–T3 spawner *necessarily* pays back fast, so accessible farming means
   a brisk first day — that is arithmetic, not a tuning choice. The pacing
   discipline therefore lives from T4 up, where values are free to scale.

3. **A lucrative product must sit behind an expensive spawner, not a long chain.**
   Optimal play duplicates the best income-per-coin line relentlessly, so anything
   cheap that feeds a valuable item bankrolls the whole tech tree. The ladder
   enforces this by construction: value rises with tier, so the gems that are
   worth hundreds sit behind the spawners that cost millions.

4. **Villagers are the strongest single force in the economy** — banked
   population multiplies sell price, offline earnings and build cost at once, so a
   buff player out-earns a pure seller by orders of magnitude and reaches the top
   tier roughly twice as fast. They are gated by *hands-on time*, not money (town
   halls bank only during active play — see `offline.ts`), which is the real
   throttle. `townScaling.diminishingExponent` (0.4) is the knob that decides how
   far the multiplier can run: at 0.5 it reached ×45 over a month and made buffs a
   mandatory bypass of the whole ladder; 0.4 lands it in single digits. Reduction
   levers are additionally kneed and floored (`townLeverFloors`,
   `townLeverAsymptotes`).

5. **The villager chain needs no gems.** A villager is `food + drink + bed`
   (`bed = planks + wool`, `drink = water`), and every specialist is
   `villager + one cheap item` (merchant←ring, guard←iron-sword, innkeeper←bed,
   mason←planks, farmer←wheat, miner←ore) — all T1–T3. Keep it that way: a
   villager recipe that needs a T5+ item re-gates the whole town behind the end
   game.

### Costing a new item

Pick the tier its bottleneck spawner sits in, then price it in that tier's band
(the table above). A processed item should clear its inputs by ~1.3× per step —
`scripts/ladder.ts` enforces that floor automatically, lifting any product whose
inputs have outgrown its authored value, so a chain can never be a loss.

An item is only "end-game" if its chain bottoms out at a **high-tier** spawner.
If it can be built from T1–T3 inputs it will be spammed early regardless of its
price — check with `npm run simulate`.

### Costing a new spawner

Add it to a tier in `scripts/ladder.ts`, give its output a value inside that
tier's band, and run `npm run ladder -- write` — the cost is derived for you.
Then **run `npm run simulate`** and read the end-game day plus `firstBuffDay` for
`Daily 1h` and `Light`: a T1–T3 addition should barely move them; a T5+ addition
should slot into the late staircase. If a cheap addition pulls the end-game day in
sharply, its output is feeding a lucrative line (#3) — raise its tier.
...
| 6 | combiner | ~$2.2k — a second spawner whose output pairs with processed ore |
| 7 | village hut | ~$249k — the hut plus a food, a drink and a bed all reachable |
| 8 | town hall | $1M, once a hut is standing |
...
combiner, and the hut card on the hut plus three feeder lines. The numbers above
move with the tier ladder, so re-check them after `npm run ladder -- write`. It over-estimates slightly on purpose (every processor step is priced
